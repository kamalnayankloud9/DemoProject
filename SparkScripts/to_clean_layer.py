...
class CleanLayer(Setup):
    ...
    def datetime_formatter(self):
        ...
        # self.clean_df = self.clean_df.withColumn("datetime",
        #                                          split(self.clean_df["datetime"], ' ').getItem(0)) \
        #                                 .withColumn("datetime", to_timestamp("datetime", 'dd/MMM/yyyy:hh:mm:ss'))
        # self.clean_df = self.clean_df.withColumn("datetime", date_format("datetime", 'dd/MMM/yyyy:hh:mm:ss'))
        #     #.withColumn("datetime", date_format("datetime", 'MM-dd-yyyy:hh:mm:ss'))
        self.clean_df.show()
