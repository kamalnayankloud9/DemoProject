...
class Agg(Curated):
    ...
    def agg_per_device(self, df_temp):
        ...
        df_with_consecutive_increasing_id = df_temp.withColumn('increasing_id', row_number().over(window))

        df_agg_per_device = df_with_consecutive_increasing_id.select("increasing_id","row_id", "day_hour", "clientip", "no_get", "no_post", "no_head") \
            .groupBy("day_hour", "clientip") \
            .agg(first("increasing_id").alias("row_id"),
        ...
        df_agg_per_device = df_agg_per_device.select("row_id","day_hour", "clientip", "no_get", "no_post", "no_head")
    ...
    def agg_across_device(self,df_temp):
        ...
        df_agg_across_device = df_with_consecutive_increasing_id.select("*") \
            .groupBy("day_hour") \
            .agg(
                first("increasing_id").alias("row_id"),
        ...
        df_agg_across_device = df_agg_across_device.select("row_id","day_hour","no_of_clients","no_get","no_post","no_head")
    ...
    def write_to_snowflake(self):
        ...
        df_per_device_sn.write.format("snowflake") \
            .options(**snowflake_options) \
            .option("dbtable", "log_agg_per_device") \
            .option("header", "true") \
            .mode("overwrite") \
            .save()
        ...
        df_across_device_sn.write.format("snowflake") \
            .options(**snowflake_options) \
            .option("dbtable", "log_agg_across_device") \
            .option("header", "true") \
            .mode("overwrite") \
            .save()
